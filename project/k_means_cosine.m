function [z, U, score] = k_means_cosine(data, nClusters, varargin)
...
while (change>threshold && iterations < maxIter),
    ...
    for k = 1:nClusters
        ...
        data_mag = sqrt(sum(data.^2, 1));
        mean_mat = repmat(U(:,k), 1, nExamples);
        mean_mag = sqrt(sum(mean_mat.^2, 1));
        mag_prod = data_mag.*mean_mag;
        Z(k,:) = ((sum(data.*repmat(U(:,k), 1, nExamples),1))./mag_prod); % Calculating cosine distance
    end
